...
int HZD_StepCheck( SVECTOR *nears, int count, int scale, SVECTOR *out )
{
    ...
    CopyToHzdVec((SCRPAD_ADDR + 0x00C), nears);
    ...
    if ( count == 2 )
    {
        CopyToHzdVec((SCRPAD_ADDR + 0x010), &nears[1]);
        area2 = Len2D((SVECTOR *)(SCRPAD_ADDR + 0x010));

        if ( area2 < scale )
        {
            Mul2D((SVECTOR *)(SCRPAD_ADDR + 0x014), (SVECTOR *)(SCRPAD_ADDR + 0x00C), scale, area);
            Mul2D((SVECTOR *)(SCRPAD_ADDR + 0x018), (SVECTOR *)(SCRPAD_ADDR + 0x010), scale, area2);
    ...
    CopyFromHzdVec(out, (SVECTOR *)(SCRPAD_ADDR + 0x014));
    return 1;
}
