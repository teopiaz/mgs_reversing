...
static struct {
    void *wall_054;         /* wall ptr in result block at 0x04C+8 (candidate) */
    void *wall_070;         /* wall ptr in result block at 0x068+8 (best) */
    void *wall_08C;         /* wall ptr in result block at 0x084+8 (2nd best) */
...
void HZD_GetNearHazard(HZD_SEG **segs)
{
    ...
    if (segs[0] && (uintptr_t)segs[0] < 0x1000) {
        printf("[HZD] WARNING: wall_070=%p looks invalid\n", segs[0]);
    }
    if (segs[1] && (uintptr_t)segs[1] < 0x1000) {
