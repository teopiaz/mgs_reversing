...
# tools/mgs_tools/cli/import_stage.py → repo root is 4 levels up.
REPO = Path(__file__).resolve().parents[3]

from mgs_tools.stage.constants       import gv_strcode, DG_MODEL_BOTHFACE, texture_slot
from mgs_tools.stage.obj_reader      import parse_obj
from mgs_tools.stage.kmd_writer      import write_kmd
from mgs_tools.stage.pcx_writer      import write_pcx
from mgs_tools.stage.hzd_writer      import write_hzd
from mgs_tools.stage.gcx_writer      import write_gcx
from mgs_tools.stage.datacnf_writer  import dar_archive, build_datacnf
...
def build_stage(name: str,
   ...
   # back-face culling silently drops half the triangles.
    ...
    if collision_path and collision_path.is_file():
        ...
        from mgs_tools.stage.obj_reader import ObjMesh, ObjFace
        collision = ObjMesh()
        collision.positions = [
    ...
    from mgs_tools.stage.hzd_writer import (    # noqa: E402
    ...
    bundled_dir = REPO / "tools" / "mgs_tools" / "stage" / "bundled"
    BUNDLED_KMD_NAMES = (
    ...
    if scenerio_path.is_file():
        from mgs_tools.cli.extract_actors import extract
...
def main():
    ...
    from mgs_tools.gcl.parser import GclSyntaxError
    try:
