...
Library code for the GCL scripting language used by MGS1 PSX stage
scripts (`*.gcx` bytecode). The user-facing CLIs live at the repo's
`tools/` (`gcl2gcx.py`, `gcx2gcl.py`, `show_glyphs.py`,
`derive_table.py`); each is a thin shim around `mgs_tools.cli.<name>`.

Supports byte-exact round-trip:
...
```
# Decompile
python3 tools/gcx2gcl.py path/to/00ea54.gcx -o /tmp/scen.gcl --trailing /tmp/scen.tail

# Recompile
python3 tools/gcl2gcx.py /tmp/scen.gcl --trailing /tmp/scen.tail -o /tmp/scen.gcx --no-align4
```
...
Labeling them lets you expand [glyphs/chars.py](glyphs/chars.py) so more
`{XXXX}` markers render as readable glyphs.

```
# ASCII art of every bank-2 glyph used in one file
python3 tools/show_glyphs.py path/to/00ea54.gcx

# Every glyph in the trailing blob (including unused slots)
python3 tools/show_glyphs.py --all path/to/00ea54.gcx

# Dedupe across all stages; write a PGM sheet + labels template
python3 tools/show_glyphs.py --scan /path/to/stages/ \
...
files, [tools/derive_table.py](../../derive_table.py) aligns the two
and produces [glyphs/chars_derived.py](glyphs/chars_derived.py) — a
`{code: char}` dict that [glyphs/chars.py](glyphs/chars.py) auto-merges
on import:
...
- [constants.py](constants.py) — `GclCode` opcodes, `GclOperator` table,
  known command hashes (`GclCommand`). `gv_strcode()` lives in
  [`mgs_tools.common.strcode`](../common/strcode.py) (single source of
  truth shared with the stage authoring side); re-exported here for
  callers that still expect `from mgs_tools.gcl.constants import gv_strcode`.
- [decompile.py](decompile.py) — bytecode → AST.
- [writer.py](writer.py) — AST → `.gcl` text.
- [parser.py](parser.py) — `.gcl` text → AST (recursive descent).
- [compile.py](compile.py) — AST → bytecode.
- [names.py](names.py) — `&NAME` symbolic constants ↔ hash.
- [glyphs/chars.py](glyphs/chars.py) — code ↔ char table (seeded from
  [font.c](../../../source/font/font.c)); grow as mappings are learned.
- [glyphs/extractor.py](glyphs/extractor.py) — bank/offset math, 12×12
  bitmap decode, PGM sheet output.

CLIs live at the repo's [tools/](../../) root:

- [tools/gcx2gcl.py](../../gcx2gcl.py) — decompile CLI
  (`mgs_tools.cli.gcx2gcl`).
- [tools/gcl2gcx.py](../../gcl2gcx.py) — compile CLI
  (`mgs_tools.cli.gcl2gcx`).
- [tools/show_glyphs.py](../../show_glyphs.py) — inspect stage glyphs
  (single file, full trailing blob, or deduped scan across many).
...
| `m"..."`         | MGS-encoded string: ASCII chars + `{XXXX}` markers for 2-byte glyph codes, or chars from the [glyphs/chars](glyphs/chars.py) table |
| `'c'`            | CHAR literal                               |
