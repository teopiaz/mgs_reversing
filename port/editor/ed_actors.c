/* Loads actor placements from a JSON (preferred) or TSV produced by
   tools/extract_gcl_actors.py and draws a small wireframe cube for each
...
static void parse_actor_options(JR *r, EdActor *a)
{
    ...
    do {
        ...
        if (!jr_eat(r, ':')) { r->err = 1; return; }
        /* The option is always an array (extract_gcl_actors lifts every
...
void ed_actors_load(const char *path)
{
     ...
     * haven't been regenerated by tools/build_editor_data.py.
     *
...
void ed_actors_render(void)
{
    for (int i = 0; i < g_actor_count; i++) {
         ...
         * actors keep their type-hash color from extract_gcl_actors. */
