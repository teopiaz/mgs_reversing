...
static int GM_Command_map(unsigned char *top)
{
    ...
    if (GCL_GetOption('a'))
    {
        ...
        {
            int val = GCL_GetNextParamValue();
            pMapRecord = GM_FindMap(val);
            if (pMapRecord != 0)
            {
                new_binds |= pMapRecord->index;
                found_any = 1;
            }
        }
        if (found_any)
            gBinds_800ABA60 = new_binds;
        /* If nothing found, keep previous binds */
    }
...
static int GM_Command_chara(int argc, char **argv)
{
    ...
    if (func != NULL)
    {
        ...
        printf("[gcl] chara: name=0x%X func=%p binds=0x%X\n", name, func, gBinds_800ABA60);
