...
/* 64-bit: the point-check result blocks store wall pointers in scratchpad int
   slots (0x054/0x070/0x08C) and pFlags in 0x0B0/0x0B4. Mirror them here so the
   full 8-byte pointers survive (memcpy of the 28-byte blocks would truncate). */
static struct {
    void *wall_054, *wall_070, *wall_08C;
    char *pt_flags_ptr;
    int   pt_flags_base;
} near_ptrs;
...
STATIC void PointTestSegment(HZD_SEG *wall, int index, int flags)
{
    ...
    *(HZD_SEG *)(SCRPAD_ADDR + 0x024) = *wall;
    ...
    ptr1[2] = 0; /* wall pointer kept in side-channel */
    near_ptrs.wall_054 = (void *)wall;
    ptr1[3] = (flags & 0x7F) | near_ptrs.pt_flags_base | (*(near_ptrs.pt_flags_ptr - index) << 8);

    if (opz < ptr2[1])
    {
        memcpy(ptr, ptr2, 28);
        memcpy(ptr2, ptr1, 28);
        near_ptrs.wall_08C = near_ptrs.wall_070;
        near_ptrs.wall_070 = near_ptrs.wall_054;
    }
    else if (*(int *)(SCRPAD_ADDR + 0x05C) != *(int *)(SCRPAD_ADDR + 0x078))
    {
        memcpy(ptr, ptr1, 28);
        near_ptrs.wall_08C = near_ptrs.wall_054;
    }
    else
    {
        return;
...
int HZD_NearHazardCheck(HZD_HDL *hzd, SVECTOR *from, int range, int chk_flag, int seg_flag)
{
    ...
    *(int *)(SCRPAD_ADDR + 0x048) = 0;
    near_ptrs.wall_054 = near_ptrs.wall_070 = near_ptrs.wall_08C = NULL;
    ...
    if (chk_flag & HZD_CHK_F_SEGMENT)
    {
        ...
        *(int *)(SCRPAD_ADDR + 0x06C) = range * range;
        ...
        near_ptrs.pt_flags_base = 0;
        near_ptrs.pt_flags_ptr = pFlags + wall_count * 2;
    ...
    if (chk_flag & HZD_CHK_D_SEGMENT)
    {
        ...
        near_ptrs.pt_flags_base = 0x80;
        near_ptrs.pt_flags_ptr = pFlags + queue_size + idx;
...
{
    segs[0] = near_ptrs.wall_070;
    segs[1] = near_ptrs.wall_08C;
}
