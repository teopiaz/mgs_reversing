...
/* 64-bit: scratchpad int slots (4 bytes) can't hold full 8-byte pointers.
   Store the line-check segment/floor pointer (slot 0x064) and pFlagsEnd
   (slot 0x070) in a side-channel; the int slots keep a truncated value. */
static struct { void *seg_064; char *flags_070; } online_ptrs;
    ...
    *(HZD_SEG *)(SCRPAD_ADDR + 0x034) = *(HZD_SEG *)floor;
    ...
    if ((flags & 1) || HZD_80027D80(floor))
    {
        ...
        *(int *)(SCRPAD_ADDR + 0x064) = (int)(intptr_t)UNTAG_PTR(HZD_FLR, floor);
        online_ptrs.seg_064 = UNTAG_PTR(HZD_FLR, floor);
    }
}
    ...
    if (chk_flag & HZD_CHK_F_SEGMENT)
    {
        ...
        for (n_areas2 = hzd->def->n_groups; n_areas2 > 0; n_areas2--, bit2 <<= 1, pArea++)
        {
            if (current_group & bit2)
            {
                ...
                online_ptrs.flags_070 = pFlagsEnd;
        ...
        while ((pNextMap = GM_IterHazard(pNextMap)))
        {
            ...
            do
            {
                ...
                *((short *)(scratchpad + 0x6A)) = 0x80;
                ...
                online_ptrs.flags_070 = pFlagsEnd2;
