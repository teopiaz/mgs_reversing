...
void d11c_800C5094(GasEfctWork *work, int arg1)
{
    ...
    /* The gas-ring quads use abr=2 (SUBTRACT) per-pixel blending with
       per-vertex colours from a GTE light pass. On PSX the GTE light pipeline
       produces a dim colour (~30 per channel) for this actor's vectors, so
       subtract creates a subtle haze. The port's GTE math produces much
       brighter colours (~240); scale the lit colour down so subtract dims
       subtly instead of clobbering the periphery to opaque black (s02b
       duct-crawl iris). Tune the shift if the haze is too strong/weak. */
    gte_ldv0(&work->f2C);
    gte_ll();
    gte_stsv(&sp30);
    sp30.vx >>= 3;
    ...
    if (!(GM_GameStatus & STATE_THERMG))
    {
        for (i = 0; i < 16; poly++, i++)
        {
            LSTORE(col0, &poly->r0);
            LSTORE(col0, &poly->r1);
            /* Preserve the hardcoded outer-vertex colour (100,100,60) from
               d11c_800C49A4 so the donut renders as a radial gradient (subtle
               inner -> darker outer), instead of every pixel subtracting the
               same lit colour. On PSX the GTE light produces a dim col1 that's
               nearly the same as col0; on the port col1 is much brighter and
               would flatten the gradient if we copied it over the hardcoded
               outer values — so the per-frame r2/r3 LSTORE the original code
               does is intentionally omitted here. */
            addPrim(ot, poly);
        }
